...
class Trainer(object):
        ...
        for step in trange(self.start_step, self.max_step):
            ...
            if step % self.save_step == self.save_step - 1:
                ...
                for iter in np.arange(self.test_iter):
                    ...
                    if iter == self.test_iter - 1:
                        fetch_dict.update({'test_summary': self.test_summary_op})
                    result = self.sess.run(fetch_dict)
                    if iter == self.test_iter - 1:
                        self.summary_writer.add_summary(result['test_summary'], step)
                        self.summary_writer.flush()
    ...
    def build_model(self):
        ...
        self.c_loss, feat, self.accuracy, self.c_var, self.x_reconstructed = self.model(
            x, self.labels, self.c_num, self.batch_size, is_train=True, reuse=False)
        self.c_loss = tf.reduce_mean(self.c_loss)
        ...
        fc4_grad = tf.reduce_max(tf.abs(tf.gradients(self.c_loss, fc4_weights)))
        ...
        self.summary_op = tf.summary.merge([
            tf.summary.scalar("tloss", self.c_loss),
            tf.summary.scalar("recloss", self.accuracy),
            tf.summary.scalar("lr", self.lr),
            tf.summary.scalar("first_grad", conv1_grad),
            tf.summary.scalar("last_grad", fc4_grad),
            # tf.summary.scalar("max_input", norm_img(tf.reduce_max(self.x))),

            tf.summary.image("inputs", self.x),
            tf.summary.image("outputs", denorm_img((self.x_reconstructed))),
            # tf.summary.image("x_grad", x_grad),

            tf.summary.histogram("latent", feat),
            tf.summary.histogram("output", self.x_reconstructed),
            tf.summary.histogram("input", self.x)
        ])
    ...
    def build_test_model(self):
        ...
        loss, self.test_feat, self.test_accuracy, var, test_x_reconstructed = self.model(
            test_x, self.test_labels, self.c_num, self.batch_size_test, is_train=False, reuse=True)

        self.test_summary_op = tf.summary.merge([
            tf.summary.scalar("test_tloss", loss),
            tf.summary.scalar("test_recloss", self.test_accuracy),
            # tf.summary.scalar("test_max_input", norm_img(tf.reduce_max(self.test_x))),

            tf.summary.image("test_inputs", self.test_x),
            tf.summary.image("test_outputs", denorm_img(test_x_reconstructed)),

            tf.summary.histogram("test_latent", self.test_feat),
            tf.summary.histogram("test_output", test_x_reconstructed),
            tf.summary.histogram("test_input", self.test_x)
        ])
